...
just build my-track-name --viz     # то же, но с линией спектра и прогресс-баром;
                                   # выбор запоминается в meta.json (video_style)
...
sovigen/
  ...
  ffmpegcmd.py      # сборка ffmpeg-команд: статичное видео и вариант с линией спектра
...
песни, `+faststart` для быстрой отдачи. Точные команды — в
`sovigen/ffmpegcmd.py`.

Видов сборки два:

- **статичный** (по умолчанию) — неподвижная обложка;
- **`freqline`** — та же обложка плюс линия спектра и прогресс-бар внизу
  кадра: `just build <slug> --viz`.

Выбор запоминается в `meta.json` в поле `video_style`, поэтому пересборка и
`build-all` повторяют его без флага. Визуализация считает все 24 кадра в
секунду, поэтому сборка идёт минуты, а не мгновенно, и файл выходит заметно
тяжелее.

Две вещи, которые в этом графе неочевидны и легко ломаются при правке:
наложение делается в RGB, а перевод в `yuv420p` стоит один раз в самом конце
(смешивание RGBA-слоя поверх YUV переставляет цветовые плоскости, и кадр
уезжает в пурпур); и диапазон явно прижимается к `tv`, иначе full-range из
JPEG протекает в поток как `yuvj420p`.
